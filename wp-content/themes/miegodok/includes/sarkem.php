...
if(! defined('ALLOW_EXTERNAL') ) 		define ('ALLOW_EXTERNAL', TRUE);			// Allow image fetching from external websites. Will check against ALLOWED_SITES if ALLOW_ALL_EXTERNAL_SITES is TRUE
...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.sarkem.txt');		// What to put at the end of all files in the cache directory so we can identify them
...
class timthumb {
	...
	public function __construct(){
		...
		$cachePrefix = ($this->isURL ? 'sarkem_ext_' : 'sarkem_int_');
		if($this->isURL){
	...
	protected function cleanCache(){
		...
		$lastCleanFile = $this->cacheDirectory . '/sarkem_cacheLastCleanTime.touch';
